...
import { getLocalStorageItemAndClear, parseArray, parseOrFallback } from "../Model/Utility/utility";
...
export default class v1Upgrader {
  ...
  migrateSettings = (): void => {
    ...
    if (firstDayOfWeek != null) {
      const parsedFirstDayOfWeek: string = parseOrFallback(firstDayOfWeek);
  ...
  // chats
  migrateChats = (): void => {
    const chatIdString: string | null = getLocalStorageItemAndClear("chat-ids");
  ...
  migrateChatById = (id: string): void => {
    ...
    // secondary channels
    const secondaryChannelString: string | null = getLocalStorageItemAndClear(
    ...
    const potentialMessagesInOutbox: any[] = parseArray(messageOutboxString);
    ...
    const addMessages = (
      ...
      for (const potentialMessage of potentialMessages) {
        ...
        const v1ChatMessage: typeof V1ChatMessageReference = potentialMessage;
